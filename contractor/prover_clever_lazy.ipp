    ...
    while(!pfn.undefined.empty()) {
        ...
        for(auto it = pfn.undefined.begin(); it != pfn.undefined.end(); ++it) {
            ...
            if(e.ys_options.empty()) {
                ...
                for(auto y : ys_all()) {
                    ...
                    for(auto f : pfn.defined)
                        if(ieq.hamming_rhs(y, f.y) >
                           ieq.hamming_lhs(e.x, f.x)) {
                            ok = false;
                            break;
                        }
                    if(ok)
                ...
                if(e.ys_options.empty()) {
                    if(made_choices) {
                        ...
                        CONTRACTOR_LOG(
                            "cannot extend further (before making any "
                            "choices): x = "
                            << e.x.str_lsb_first());
                        throw std::runtime_error(
                            "cannot extend further (before "
                            "making any choices)");
                    }
                }
            }
            ...
            if(e.ys_options.size() == 1) {
                new_entry = defined_entry{e.x, e.ys_options[0],
        ...
        if(!found_unique_extension) {
            ...
            CONTRACTOR_LOG("    made a choice: x = "
                           << new_entry.x.str_lsb_first()
                           << ", f(x) = " << new_entry.y.str_lsb_first());
        }
